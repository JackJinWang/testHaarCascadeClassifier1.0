	...
	int maxtreesplits, int minpos, bool bg_vecfile,bool pos_vecfile)
{
   ...
   //读入XML
	...
	if (files.size() > 0)
	{
		...
		strongFile = files.size();
	}
	else
	{
		printf("Unable to read XML");
		__MY_EXIT__
	}
	predit_result = new int[sampleNumber * strongFile]; //阶段预测
	for (int i = 0;i < files.size();i++)
	{
		strongClassifier[i] = readXML(files[i].c_str(), strongClassifier[i]);
	}
	for (int i = 0; i < strongFile;i++)
	{
		predict(&predit_result[i * sampleNumber], sampleNumber, haar_features, training_data, strongClassifier[i]);
	}
	
	for (int i = 0;i < sampleNumber;i++)
	{
		int re = 1;
		for (int m = 0;m < strongFile;m++)
		{
			re = re&predit_result[i + m*sampleNumber];
		}
		if ((re == 1) && (training_data->cls.data.fl[i] == 1.0))
			hitRate_real++;
		else if ((re == 1) && (training_data->cls.data.fl[i] == 0.0))
			maxFalse_real++;
		else if ((re == 0) && (training_data->cls.data.fl[i] == 0.0))
			negHit++;
